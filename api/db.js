...
export default async function handler(req, res) {
  ...
  try {
    ...
    switch (action) {
      ...
      // ============ SAI PIN (Hidden Category Access) ============
      case 'getSaiPinStatus':
        // Check if SAI PIN is set up
        const [saiStatus] = await sql`
          SELECT sai_pin_hash FROM app_settings WHERE id = 'main' LIMIT 1
        `
        return res.json({ data: { exists: !!saiStatus?.sai_pin_hash } })

      case 'setupSaiPIN':
        // Setup SAI PIN (first time)
