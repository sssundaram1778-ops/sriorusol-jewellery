  ...
  if (sql) {
    try {
      const pledges = await sql`
        SELECT * FROM pledges WHERE status = 'ACTIVE' ORDER BY date DESC, pledge_no DESC
      `
      ...
      const ownerRepledges = await sql`
        SELECT pledge_id, financer_name FROM owner_repledges WHERE pledge_id = ANY(${pledgeIds}) AND status = 'ACTIVE'
      `
...
export const getClosedPledges = async () => {
  if (sql) {
    try {
      const pledges = await sql`
        SELECT * FROM pledges WHERE status IN ('CLOSED', 'REPLEDGED') ORDER BY date DESC, pledge_no DESC
      `
      ...
      const ownerRepledges = await sql`
        SELECT pledge_id, financer_name FROM owner_repledges WHERE pledge_id = ANY(${pledgeIds})
      `
...
export const getAllPledges = async () => {
  if (sql) {
    try {
      const pledges = await sql`SELECT * FROM pledges ORDER BY date DESC, pledge_no DESC`
      ...
      const ownerRepledges = await sql`
        SELECT pledge_id, financer_name, status FROM owner_repledges WHERE pledge_id = ANY(${pledgeIds})
      `
      ...
      return pledges.map(pledge => {
        ...
        // For active pledges show active financer, for closed show any financer
  ...
  if (sql) {
    try {
      ...
      return (pledges || []).map(pledge => {
        ...
        // For closed pledges use canceled_date, for active use current date
        const endDate = (pledge.status === 'CLOSED' || pledge.status === 'REPLEDGED') && pledge.canceled_date 
          ? new Date(pledge.canceled_date) 
          : new Date()
...
export const getActivePledgesWithAmounts = async () => {
  if (sql) {
    try {
      ...
      return (pledges || []).map(pledge => {
        const pledgeAmounts = (amounts || []).filter(a => a.pledge_id === pledge.id)
        // Active pledges: calculate interest till today
...
export const getClosedPledgesWithAmounts = async () => {
  if (sql) {
    try {
      ...
      return (pledges || []).map(pledge => {
        const pledgeAmounts = (amounts || []).filter(a => a.pledge_id === pledge.id)
        // Use canceled_date for interest calculation, fallback to current date if not set
