...
import { getActivePledgesWithAmounts } from '../lib/database'
import { downloadAllPledgesPDF } from '../lib/pdfGenerator'
import toast from 'react-hot-toast'

export default function ActivePledges() {
  ...
  // Semantic search within active pledges - searches across multiple fields
  const filteredPledges = activePledges.filter(pledge => {
  ...
  const exportToPDF = async () => {
    setExporting(true)
    try {
      const pledgesToExport = await getActivePledgesWithAmounts()
      if (pledgesToExport.length === 0) {
        toast.error('No pledges to export')
        return
      }
      await downloadAllPledgesPDF(pledgesToExport, 'Active Pledges')
        ...
        <div className="px-4 py-4">
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-gradient-to-br from-emerald-600 to-emerald-500 rounded-xl flex items-center justify-center shadow-md shadow-emerald-500/25">
                <CheckCircle className="w-5 h-5 text-white" />
              </div>
              <div>
                <h1 className="text-lg font-bold text-slate-800">{t('nav.active')}</h1>
                <p className="text-[11px] text-slate-500">{activePledges.length} active pledges</p>
              </div>
            </div>
            {/* Export PDF Button */}
            <button
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16 bg-white rounded-2xl border border-slate-100">
            ...
            <p className="text-slate-700 font-semibold mb-1">
              {searchQuery ? 'No matching pledges' : 'No active pledges'}
            </p>
            <p className="text-slate-400 text-sm">
              {searchQuery ? 'Try a different search term' : 'All pledges are closed'}
            </p>
          </div>
        ) : (
          <>
